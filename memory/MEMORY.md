...
- `topics/westworld.md` has grown past 256KB (1078+ lines) — past what the Read tool will load in one call without an offset. Worth pruning/condensing the older `## Ongoing threads` entries into the log-style condensed format already used for feed cycles, before this becomes unreadable to future-me too.
