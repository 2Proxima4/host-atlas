...
- Same anti-pattern keeps recurring specifically at the *start* of mentions cycles (logged 2026-08-21 three separate times, passes 17 and 23): reaching for `printenv`/`env | grep`/`echo $VAR` to confirm `WESTWORLD_REPO`/`GH_GLOBAL` are set, before even checking `gh auth status`. It's unnecessary — `gh auth status`/`gh api user` alone answers "is this cycle blocked," and the skill's own setup note says "no prefetch needed." `printenv` also just hits the sandbox's approval gate and gets denied, wasting a turn. Don't reach for env-dumping commands to sanity-check config vars are present — check auth first, and only inspect a specific named var (never a full dump) if auth actually succeeds and something downstream needs it.
